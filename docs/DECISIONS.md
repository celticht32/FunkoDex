...
**Status:** Active — PARTIALLY FULFILLED by DEC-032 (S24): the runtime half exists (`FunkoRepository.normaliseForSave` re-homes and logs, and deletes the stale original). The build-failing unit test this entry asks for is still owed.
...
### DEC-031: A catalog record needs a usable title; degenerate titles are qualified, not discarded (S24)
**Status:** Active — implemented
**Context:** `FunkoLookupService` read titles in two places that both assumed a usable value: `catalogDocToFunkoItem` set `name = doc.getString("title") ?: ""`, and `searchLocalByName`'s refine matched against `"$title $series"`. A record with a blank or single-character title therefore rendered nameless on a UPC scan and dropped out of name search entirely. Measured on the 2026-09-12 device backup: 11 such records, and 0 affected owned items. The shipped asset carries only 2 (`v`, `l`); the other 9 arrived via on-device enriched imports and are PriceCharting-sourced, so their handles are opaque ids (`pc-7514981`) and their `series` holds the Pop! LINE ("Pop! Rocks"), not the property.
**Decision:** A shared `data/preload/TitleDq` owns the rule. `isUsableTitle` = at least two alphanumerics after trimming. On the READ side, `displayName` keeps a usable title; QUALIFIES a degenerate-but-present one with the record's property from `pcSeries`/`franchiseSuggestion` ("V" -> "V (BTS)"), which is the same convention used when repairing these at source; and only then falls back to property, series, de-slugged handle. `searchHaystack` combines title + series + property + de-slugged handle, and the coarse Couchbase LIKE pre-filter must test the SAME columns or a record never reaches the refine. On the WRITE side, `CatalogImporter` refuses a net-new record whose title fails `isUsableTitle`, counted separately as `skippedBlankTitle` and surfaced in the import dialog. Merges are NOT filtered — audit confirmed `mergeRecordInto` never writes `FIELD_TITLE`, so a blank incoming title cannot overwrite a good stored one.
**Consequences:** Verified against real data: distinct display names for the 11 went from 4 to 7 (the four remaining collisions are genuinely four different BTS "V" figures, separated on screen by their Pop number). Searching "BTS" now reaches them, which it could not before — that required adding `pcSeries` to the pre-filter, not just to the haystack. This is latent-hazard work: it exists for the externally-authored records that §7 Community Catalog Distribution will ingest. 24 unit tests in `TitleDqTest`. Note the tension with DEC-021 (no-identity rows are filtered, never deleted): the insert guard REFUSES data rather than filtering it at the surface. Accepted on the grounds that an externally-authored record with no usable title is junk rather than a preservable Pocket Pop — but it is a deliberate departure, not an oversight.

### DEC-032: The collection-item id invariant is enforced at the write boundary, and catalog rows are read-only on the browse path (S24)
**Status:** Active — implemented (partially fulfils DEC-018)
**Context:** Third recurrence of the DEC-007b/DEC-018 corruption. S17 repaired 8 documents; S18 repaired 86 and guarded `DetailViewModel.toggleOwned()`; the 2026-09-12 backup then showed 11 more, ALL added on two dates (2026-08-02, 2026-08-09) and 10 of the 11 carrying the catalog row's own `lastUpdated` — i.e. catalog rows adopted wholesale. Instrumenting `saveItem` identified the real path, and it was never `toggleOwned`: every warning was immediately preceded by `PriceService: Tier 1m (PriceCharting) hit for <name>`. `DetailViewModel.refreshPrices` writes the item back when the resolved price differs, and it is called from `loadItem` — so merely OPENING a catalog-backed figure persisted it. Both previous fixes were in CALLERS, which is why neither held: `FunkoLookupService.catalogDocToFunkoItem` sets `id = docId`, so EVERY item from a catalog lookup or name search arrives at the repository carrying a `catalog::` id.
**Decision:** Two layers. (1) INVARIANT, at the single write boundary: `FunkoRepository.saveItem` routes through `normaliseForSave`, which re-homes any `catalog::` id to `funko::{upc|uuid}`, preserves the severed link into `catalogRef` only when that field is blank (4 of the 11 already pointed at a better-matched row), logs at WARN naming the offending id, and then DELETES the document left at the old id — but only when its stored type is `funko`, never when it is `catalog`, since that is reference data the new record now links to. Moving rather than copying matters: the first cut copied, and owned count went 373 -> 374 with both ids live for one figure. (2) BEHAVIOUR: `refreshPrices` persists only for a collection record — `id.startsWith("funko::") || isOwned`. The ownership half is load-bearing: without it a corrupted owned record would be treated as reference data and could never heal. The resolved price is still displayed and still cached as a `price::` snapshot; only the write onto the item is suppressed.
**Consequences:** Guarding one caller cannot hold an invariant — the guard belongs where the write happens. Observed working: four records re-homed live during testing simply by opening them. Force-restoring an unrepaired backup re-injects all 11, so the backup remains the carrier until repaired; that repair is a standalone script, deliberately NOT an in-app migration (one-time migrations become permanent cruft). DEC-018's build-failing unit test is still outstanding — this is the runtime half only.

### DEC-033: An unparseable price is refused, never written as zero (S24)
**Status:** Active — implemented
**Context:** Four sites shared `raw.replace(Regex("[^0-9.]"), "").toDoubleOrNull() ?: 0.0`. It handles "$1,234.56" but mangles any value with a second separator: "$10.00 - $15.00" becomes "10.0015.00" and "1.234,56" becomes "1.23456". Both fail `toDoubleOrNull()`, and the call sites substituted 0.0 — a guess, and one indistinguishable from a genuinely free item. `MoneyInput.sanitize` already solved this for hand-typed input; the catalog-side parser never got the same treatment.
**Decision:** `util/PriceParse` counts money tokens: exactly one parses, zero or more than one returns null. A range has no single correct answer and picking an end is precisely the guess DEC-025 forbids, so it refuses; a European "1.234,56" also yields two tokens and is refused rather than read as 1.234. Null never means zero — a returned 0.0 means the string genuinely said zero. Fill-only callers (`CatalogImporter` merge, `CollectionRelinkService`) leave the stored value untouched on null. The insert path keeps `?: 0.0` because a new record has nothing to protect and 0.0 already means "no retail price recorded".
**Consequences:** Extends DEC-025 from enrichment into the app's parsing layer. Latent today — current enricher output is clean — and fires when externally-authored data arrives, the same trigger as DEC-031. 7 unit tests in `PriceParseTest`.
