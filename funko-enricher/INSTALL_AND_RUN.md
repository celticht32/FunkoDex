...
Windows 11: right-click `funko-enricher.tar.gz` in File Explorer → **Extract All**.

Windows 10 or if extraction fails: use [7-Zip](https://www.7-zip.org) (free).
Right-click → **7-Zip → Extract Here**.

You should end up with a `funko-enricher` folder containing:
...
## Step 5b — Confirm Chrome is installed (for Pass 2)

Pass 2 scrapes funko.com using a real browser to bypass bot detection.
It auto-detects Chrome from the standard install location. If you have
Google Chrome installed you're already set — skip ahead.
...
The enricher runs in three passes. You can run all three at once or skip passes
you don't need yet.
...
```
node enrich.js --input funko_data_enriched.json --output funko_data_enriched.json --skip-kenny --skip-funko --pc-limit 500
```
...
Running Pass 3 overnight with a high limit is a valid strategy.
...
```
Pass 1 and 2 will add new items; Pass 3 will only look up records still
missing pricing from previous runs.
